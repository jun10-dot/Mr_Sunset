...
void GameManager::ShowScore()
{
	...
	if (!timerStarted)
	{
		// 적이 모두 죽거나 없고, 총알 발사 중단된 상태라면 (성공 조건)
		...
		// 적이 남아있고 총알 6발 다 쓰고 
		// 총알 발사 중단된 상태인 경우 (!isFireBullet) (실패/재도전 조건)
...
void GameManager::HandleDeadEnemies()
{
	...
	for (auto enemy = myList.begin(); enemy != myList.end();)
	{
		...
		{
			if ((*enemy)->HitTime == 0) // 처음 맞은 시간 기록
			{
			...
			{
				delete *enemy; // 메모리 해제
				enemy = myList.erase(enemy);  // 리스트에서 제거
				continue;
			}
		}
		(*enemy)->Update(); // 적 업데이트
		++enemy; // 죽지 않았다면 다음 요소로 이동
