...
void Sprite::Draw( float dx , float dy , LONG sx , LONG sy , LONG sw , LONG sh, float centerX, float centerY ) // 화면의 dx, dy에
...
void Sprite::Render( float x , float y , float radian, float sx, float sy, int pivotMode, D3DCOLOR tintColor) // 회전, 확대 출력
{                                                                       
    RECT Rect ;                                                          
    ID3DXSprite* pSprite ;                                                
    ...
    {
        center.x = imagesinfo.Width / 1.9f;
        center.y = imagesinfo.Height / 1.9f;
    }
    center.z = 1.0f;
    ...
    dv_font.Sprite->Begin(D3DXSPRITE_ALPHABLEND) ;
    pSprite->Draw( Texture , &Rect, &center, NULL , tintColor) ;
    pSprite->Flush() ;
    dv_font.Sprite->End() ;
